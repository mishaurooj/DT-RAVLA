...
"""
DT-RAVLA consistent pretrained evaluation pipeline.

Purpose
-------
This script evaluates existing pretrained models under a consistent
paper protocol. Table 2 contains only the supervised SWaT benchmark, where
Logistic Regression, Random Forest, and supervised DT-RAVLA use the same
features, windows, validation set, and test set. WADI is excluded from the
benchmark and is used only as the target process in Table 4 cross-process
generalisation. Table 3 contains true SWaT ablations, Table 5 contains SWaT
robustness, and Table 6 reports the efficiency of supervised DT-RAVLA.
The script never retrains or modifies a checkpoint.
...
Required LaTeX package for the merged table
-------------------------------------------
...
def aggregate_mean_std(
    ...
    runs = grouped.size().rename("Runs")
    return pd.concat([mean, std, runs], axis=1).reset_index()
...
def save_consistent_benchmark_latex(
    summary: pd.DataFrame,
    path: Path,
) -> None:
    """
    Table 2 contains only SWaT supervised models.

    Deterministic pretrained baselines are printed as single values.
    DT-RAVLA is printed as mean +/- standard deviation over five seeds.
    This avoids fabricating seed variation for one saved baseline model.
    """
    metrics = [
        "Accuracy",
        "Balanced_Accuracy",
    ...
    lines = [
        ...
        r"\centering",
        (
            r"\caption{Supervised SWaT benchmark under one frozen preprocessing, "
            r"windowing, validation, and test protocol. Deterministic pretrained "
            r"baselines are reported once; DT-RAVLA is reported as mean "
            r"$\pm$ standard deviation over five seeds.}"
        ),
        r"\label{tab:swat_supervised_benchmark}",
        r"\renewcommand{\arraystretch}{1.12}",
        r"\setlength{\tabcolsep}{4pt}",
    ...
    order = {
        ...
        "RandomForest": 1,
        "DT-RAVLA-Supervised": 2,
    }
    summary = summary.copy()
    summary["_order"] = summary["Model"].map(order).fillna(99)
    summary = summary.sort_values("_order")

    display_names = {
        "LogisticRegression": "Logistic Regression",
        "RandomForest": "Random Forest",
        "DT-RAVLA-Supervised": "DT-RAVLA",
    }

    for _, row in summary.iterrows():
        runs = int(row["Runs"])
        values = [display_names.get(str(row["Model"]), str(row["Model"])), str(runs)]
        for metric in metrics:
            ...
            if runs > 1:
                ...
                values.append("--" if not np.isfinite(mean) else f"{mean:.3f}")
        lines.append(" & ".join(values) + r" \\")
...
def save_merged_results_subtables(
    benchmark_summary: pd.DataFrame,
    ablation_summary: Optional[pd.DataFrame],
    ...
    """
    Export one IEEE table* containing three aligned subtables.

    Subtable (a) uses the same classification fields for the supervised
    benchmark and architectural ablations.

    Subtable (b) uses the same transfer/robustness fields for cross-process
    calibration and input corruption.

    Subtable (c) reports efficiency fields, which are dimensionally different
    and therefore remain in a separate subtable inside the same table float.
    """
    lines = [
        r"% Add \usepackage{subcaption} in the document preamble.",
        r"\begin{table*}[t]",
        r"\centering",
        r"\caption{Comprehensive evaluation of DT-RAVLA under consistent benchmark, ablation, generalisation, robustness, and efficiency protocols.}",
        r"\label{tab:comprehensive_results}",
        r"\renewcommand{\arraystretch}{1.10}",
        r"\setlength{\tabcolsep}{3.2pt}",
        "",
        r"\begin{subtable}{\textwidth}",
        r"\centering",
        r"\caption{Supervised SWaT benchmark and true architectural ablations.}",
        r"\label{tab:benchmark_ablation_subtable}",
        r"\begin{tabular}{llcccccc}",
        r"\hline",
        r"Group & Configuration & BAcc. & Precision & Recall & F1 & AUPRC & MCC \\",
        r"\hline",
    ]
    ...
    benchmark = benchmark_summary.copy()
    benchmark["_order"] = benchmark["Model"].map(benchmark_order).fillna(99)
    benchmark = benchmark.sort_values("_order")
    ...
    for _, row in benchmark.iterrows():
        runs = int(row["Runs"])
        values = ["Benchmark", display_names.get(str(row["Model"]), str(row["Model"]))]
        for metric in metrics_a:
            ...
            values.append(
                ...
                if runs > 1
                else ("--" if not np.isfinite(mean) else f"{mean:.3f}")
            )
        lines.append(" & ".join(values) + r" \\")
    ...
    if ablation_summary is not None and not ablation_summary.empty:
        ...
        ablation = ablation_summary.copy()
        ablation["_order"] = pd.Categorical(
            ablation["Configuration"],
            categories=ablation_order,
            ordered=True,
        )
        ablation = ablation.sort_values("_order")

        for _, row in ablation.iterrows():
            values = ["Ablation", str(row["Configuration"])]
            for metric in metrics_a:
                values.append(
                    format_mean_std(
    ...
    lines.extend([
        ...
        r"\begin{subtable}{\textwidth}",
        r"\centering",
        r"\caption{Cross-process generalisation and corruption robustness using common detection fields.}",
        r"\label{tab:generalisation_robustness_subtable}",
        r"\begin{tabular}{lllccccc}",
        r"\hline",
        r"Group & Setting & Condition & Precision & Recall & Specificity & F1 & MCC \\",
        r"\hline",
    ])

    metrics_b = ["Precision", "Recall_DR", "Specificity", "F1", "MCC"]

    transfer = transfer_summary.copy()
    transfer_order = {
        "Zero-shot source threshold": 0,
        "Target-normal calibrated": 1,
        "One-percent few-shot calibration": 2,
    }
    transfer["_order"] = transfer["Protocol"].map(transfer_order).fillna(99)
    transfer = transfer.sort_values("_order")

    for _, row in transfer.iterrows():
        values = [
            "Generalisation",
            str(row["Direction"]).replace("_", r"\_"),
            str(row["Protocol"]),
        ]
    ...
    robustness = robustness_summary.copy()
    robustness = robustness[
        (robustness["Dataset"] == "SWaT")
        & (robustness["Model"] == "DT-RAVLA-Supervised")
    ].copy()
    robustness_order = {name: index for index, name in enumerate(ROBUSTNESS_CONDITIONS)}
    robustness["_order"] = robustness["Condition"].map(robustness_order).fillna(99)
    robustness = robustness.sort_values("_order")

    for _, row in robustness.iterrows():
        values = [
            "Robustness",
            "SWaT",
            str(row["Condition"]).replace("_", r"\_"),
        ]
        for metric in metrics_b:
            values.append(
                format_mean_std(
    ...
    lines.extend([
        ...
        r"\vspace{1.5mm}",
        "",
        r"\begin{subtable}{0.78\textwidth}",
        r"\centering",
        r"\caption{Supervised DT-RAVLA computational efficiency.}",
        r"\label{tab:efficiency_subtable}",
        r"\begin{tabular}{lcccc}",
        r"\hline",
        r"Model & Parameters & Size (MB) & Latency (ms) & Windows/s \\",
        r"\hline",
    ])
    ...
    for _, row in efficiency_summary.iterrows():
        values = [
            "DT-RAVLA",
            f"{float(row['Parameters_Mean']):,.0f}",
            format_mean_std(
                float(row["Model_Size_MB_Mean"]),
                float(row["Model_Size_MB_Std"]),
            ),
            format_mean_std(
                float(row["Latency_ms_per_window_Mean"]),
                float(row["Latency_ms_per_window_Std"]),
            ),
            format_mean_std(
                float(row["Windows_per_second_Mean"]),
                float(row["Windows_per_second_Std"]),
            ),
        ]
        lines.append(" & ".join(values) + r" \\")
...
def main() -> None:
    ...
    benchmark_raw = benchmark_raw[
        (benchmark_raw["Dataset"] == "SWaT")
        & benchmark_raw["Model"].isin(
            ["LogisticRegression", "RandomForest", "DT-RAVLA-Supervised"]
        )
    ].copy()
    benchmark_raw.to_csv(
        TABLE_DIR / "Table_2_SWaT_Supervised_Benchmark_Raw.csv",
        index=False,
    )
    ...
    benchmark_summary.to_csv(
        TABLE_DIR / "Table_2_SWaT_Supervised_Benchmark.csv",
        index=False,
    )
    save_consistent_benchmark_latex(
        benchmark_summary,
        TABLE_DIR / "Table_2_SWaT_Supervised_Benchmark.tex",
    )

    # -------------------------------------------------------------------------
    # Save Table 4: cross-process generalisation
    # -------------------------------------------------------------------------
    transfer_raw = pd.DataFrame(transfer_rows)
    transfer_raw.to_csv(
        TABLE_DIR / "Table_4_Cross_Process_Raw_All_Seeds.csv",
        index=False,
    )
    ...
    transfer_summary.to_csv(
        TABLE_DIR / "Table_4_Cross_Process_Mean_Std.csv",
        index=False,
    )
    save_latex_table(
        transfer_summary,
        ...
        "tab:cross_process_generalisation",
        TABLE_DIR / "Table_4_Cross_Process.tex",
    )

    # -------------------------------------------------------------------------
    # Save Table 5: supervised SWaT robustness
    # -------------------------------------------------------------------------
    robustness_raw = pd.DataFrame(robustness_rows)
    robustness_raw = robustness_raw[
        (robustness_raw["Dataset"] == "SWaT")
        & (robustness_raw["Model"] == "DT-RAVLA-Supervised")
    ].copy()
    robustness_raw.to_csv(
        TABLE_DIR / "Table_5_Robustness_Raw_All_Seeds.csv",
        index=False,
    )
    ...
    robustness_summary.to_csv(
        TABLE_DIR / "Table_5_Robustness_Mean_Std.csv",
        index=False,
    )
    save_latex_table(
        robustness_summary,
        ...
        "tab:robustness",
        TABLE_DIR / "Table_5_Robustness.tex",
    )
    ...
    # -------------------------------------------------------------------------
    # Save Table 6: supervised SWaT efficiency
    # -------------------------------------------------------------------------
    efficiency_raw = pd.DataFrame(efficiency_rows)
    efficiency_raw = efficiency_raw[
        (efficiency_raw["Dataset"] == "SWaT")
        & (efficiency_raw["Model"] == "DT-RAVLA-Supervised")
    ].copy()
    efficiency_raw.to_csv(
        TABLE_DIR / "Table_6_Efficiency_Raw_All_Seeds.csv",
        index=False,
    )
    ...
    efficiency_summary.to_csv(
        TABLE_DIR / "Table_6_Efficiency_Mean_Std.csv",
        index=False,
    )
    save_latex_table(
        efficiency_summary,
        ...
        "Supervised DT-RAVLA model size and inference efficiency measured on the evaluation device.",
        "tab:efficiency",
        TABLE_DIR / "Table_6_Efficiency.tex",
    )

    # -------------------------------------------------------------------------
    # Save Table 3: true SWaT ablations, when checkpoints exist
    # -------------------------------------------------------------------------
    ...
    if ablation_rows:
        ...
        ablation_raw.to_csv(
            TABLE_DIR / "Table_3_Ablation_Raw_All_Seeds.csv",
            index=False,
        )
        ...
        ablation_summary.to_csv(
            TABLE_DIR / "Table_3_Ablation_Mean_Std.csv",
            index=False,
        )
        save_latex_table(
            ablation_summary,
            ...
            "tab:true_ablation",
            TABLE_DIR / "Table_3_Ablation.tex",
        )
    ...
    # -------------------------------------------------------------------------
    # Export one merged IEEE table with aligned subtables
    # -------------------------------------------------------------------------
    save_merged_results_subtables(
        benchmark_summary=benchmark_summary,
        ablation_summary=ablation_summary,
        transfer_summary=transfer_summary,
        robustness_summary=robustness_summary,
        efficiency_summary=efficiency_summary,
        path=TABLE_DIR / "Table_2_Comprehensive_Results_with_Subtables.tex",
    )
    ...
    neural_summary = benchmark_summary[
        (benchmark_summary["Dataset"] == "SWaT")
        & (benchmark_summary["Model"] == "DT-RAVLA-Supervised")
    ].copy()
    ...
    manifest = {
        ...
        "Paper_Table_Policy": {
            "Main_Table": "One comprehensive table with three aligned subtables",
            "Subtable_a": "SWaT supervised benchmark and true ablations using identical classification fields",
            "Subtable_b": "SWaT-to-WADI generalisation and SWaT robustness using identical detection fields",
            "Subtable_c": "Supervised DT-RAVLA computational efficiency",
            "WADI_OneClass_In_Benchmark": False,
        },
        "Files": sorted(
    ...
    print("\nConsistent pretrained reevaluation with merged subtables completed.")
